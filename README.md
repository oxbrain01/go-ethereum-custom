...
# Dump configuration with specific flags applied
$ geth --http --http.port 8545 --cache 4096 dumpconfig > config.toml
```
...
```shell
# Using scripts/start-prod directory structure
# Note: Run this from the scripts/start-prod directory
cd scripts/start-prod
docker run -d --name ethereum-node \
           -v "$(pwd)/data:/root/.ethereum" \
           -v "$(pwd)/config.toml:/root/config.toml:ro" \
           -p 8545:8545 -p 8546:8546 -p 30303:30303 -p 8551:8551 \
           ethereum/client-go --config /root/config.toml --datadir /root/.ethereum
```

This will start `geth` in snap-sync mode with a DB memory allowance of 1GB, as the
above command does. It will use the `scripts/start-prod/data` directory for persistent
blockchain storage and `scripts/start-prod/config.toml` for configuration. The ports
mapped are: HTTP RPC (8545), WebSocket (8546), P2P (30303), and Auth RPC (8551).
There is also an `alpine` tag available for a slim version of the image.
